...
UPDATE departments SET budget = budget * 0.9 WHERE code = ( SELECT code FROM departments ORDER BY budget ASC LIMIT 1);

SELECT code, name FROM departments WHERE name IN ('Research', 'IT');
...
DELETE FROM employees WHERE department = (SELECT code FROM departments WHERE name = 'IT') RETURNING *;
